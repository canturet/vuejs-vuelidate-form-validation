        ...
        <form style="width: 350px" @submit.prevent="onSubmit">
          <div class="form-group">
            ...
            <small v-if="!$v.email.email" class="form-text text-danger"
              >Geçerli bir e-posta adresi girin</small
            >
          </div>
          <div class="form-group">
            <label>Şifre</label>
            ...
            <small v-if="!$v.password.minLength" class="form-text text-danger"
              >Şifreniz en az {{ $v.password.$params.minLength.min }} rakamdan
              oluşmalıdır</small
            >

            <small v-if="!$v.password.maxLength" class="form-text text-danger"
              >Şifreniz en çok {{ $v.password.$params.maxLength.max }} rakamdan
              oluşmalıdır</small
            >
          </div>
          <div class="form-group">
            ...
            <small v-if="!$v.repassword.minLength" class="form-text text-danger"
              >Şifreniz en az {{ $v.repassword.$params.minLength.min }} rakamdan
              oluşmalıdır</small
            >

            <small v-if="!$v.repassword.maxLength" class="form-text text-danger"
              >Şifreniz en çok
              {{ $v.repassword.$params.maxLength.max }} rakamdan
              oluşmalıdır</small
            >
          ...
          <div class="form-group">
            ...
            <small v-if="!$v.age.numeric" class="form-text text-danger"
              >Yaşınız rakamlardan oluşmalıdır</small
            >
            <small v-if="!$v.age.between" class="form-text text-danger"
              >Sisteme kayıt olabilmeniz için {{ $v.age.$params.between.min }} -
              {{ $v.age.$params.between.max }} yaş aralığında olmalısınız</small
            >
          </div>
          ...
          <small v-if="!$v.hobbies.minLength" class="form-text text-danger"
            >İlgi alanlarınız en az {{ $v.hobbies.$params.minLength.min }} tane
            olmalıdır</small
          >
    ...
    email: {
      ...
      uniq: (value) => {
        return new Promise((resolve, reject) => {
          setTimeout(() => {
            resolve(value !== "turetcan@gmail.com");
          }, 1000);
        });
      },
    },
